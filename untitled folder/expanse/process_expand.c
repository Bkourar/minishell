...
static t_exp	*concatition(t_exp **lst, char *str)
{
	...
	while (nd1)
	{
		...
		if (valid_join(nd1))
		{
			...
			ft_lstadd_back_texp(&head, config(str));
		}
		else
...
static char	**result_expand(t_exp **hd)
{
	...
	spl = (char **)ft_allocate(1, (ft_lstsize_exp((*hd)) + 1) * sizeof(char *));
	if (!spl)
		return (write(2, "faile allocation\n", 18), NULL);
	i = 0;
	tp = (*hd);
	while (tp)
	{
		if (!ft_strcmp(tp->input, " ") || !ft_strcmp(tp->input, "\0"))
			tp = tp->next;
		else
		{
...
char	**pi_processing_expand(char *str, t_env **env, bool b)
{
	...
	// if (b == false)
	// 	free(str);
	head = update_list(update_array(&head, 0, b), &head, b);
	if (!head)
		return (NULL);
